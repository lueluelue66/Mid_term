...
def _train(dataset_name: str, backbone_name: str, path_to_data_dir: str, path_to_checkpoints_dir: str, path_to_resuming_checkpoint: Optional[str]):
    ...
    scheduler = MultiStepLR(optimizer, milestones=[10,20], gamma=Config.STEP_LR_GAMMA)
    ...
    while not should_stop:
        ...
        for _, (_, image_batch, _, bboxes_batch, labels_batch) in enumerate(dataloader):
            ...
            if step % num_steps_to_display == 0:
                ...
                Log.i(f'[Epoch {i_epoch} Step {i_step}] Avg. Loss = {avg_loss:.6f}, Learning Rate = {lr:.8f} ({samples_per_sec:.2f} samples/sec; ETA {eta:.1f} hrs)')
...
if __name__ == '__main__':
    def main():
        ...
        parser.add_argument('-s', '--dataset', type=str, default='voc2007', choices=DatasetBase.OPTIONS, required=False, help='name of dataset')
        parser.add_argument('-b', '--backbone', type=str, default='resnet101', choices=BackboneBase.OPTIONS, required=False, help='name of backbone model')
